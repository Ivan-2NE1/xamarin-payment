...
namespace QuickDate.Activities.Favorite.Adapters
{
    public class FavoriteUsersAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
            ...
            try
            {
                if (viewHolder is FavoriteUsersAdapterViewHolder holder)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        holder.Name.Text = IMethods.Fun_String.SubStringCutOf(QuickDateTools.GetNameFinal(item), 14);
        ...
        private void FavoriteButtonClick(FavoriteUsersClickEventArgs e)
        {
            try
            {
                ...
                { 
                  ...
                  var countList =  HomeActivity?.ProfileFragment?.FavoriteFragment?.FavoriteAdapter?.ItemCount;
                  if (countList == 0)
                  {
                      HomeActivity?.ProfileFragment?.FavoriteFragment?.ShowEmptyState();
                  }
                }
            }
        ...
        public override long GetItemId(int position)
        {
            try
            {
                return FavoritesList[position].Id;
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
