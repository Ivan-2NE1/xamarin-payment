...
namespace QuickDate.Activities.UserProfile
{
    ...
    public class UserProfileActivity : AppCompatActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        public TextView Username, IconOnline, IconMore, IconFavorite, IconLocation, LocationTextView, Description, DescriptionInterest, DescriptionLanguages, DescriptionWork;
        public ViewPager ViewPagerView;
        public CircleIndicator CircleIndicatorView;
        public ImageView IconBack;
        public Button GiftButton;
        public CircleButton LikeButton, DesLikeButton, ChatButton;
        public Button WebsiteButton, SocialGoogle, SocialFacebook, SocialTwitter, SocialLinkedIn, SocialInstagram;
        public RelativeLayout SocialLayout, WorkAndEducationLayout, IntersetLayout, LanguagesLayout, FooterButtonSection;
        public UserInfoObject DataUser;
        public List<string> ListImageUser = new List<string>();
        public string EventType, LinkedIn, Twitter, Facebook, Google, Instagrem, Website;
        ...
        private void DesLikeButtonOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (EventType == "Close" || EventType == "likeAndClose")
                {
                    Finish();
                }
                else
                {
                    var list = GlobalContext?.CardFragment?.CardDateAdapter?.UsersDateList;
            ...
            try
            {
                ...
                switch (EventType)
                {
                    case "Close":
                        ...
                        break;
                    case "likeAndClose":
                        new DialogController(this).OpenDialogMatchFound(DataUser);
                        break;
                    default:
                    {
                        var list = GlobalContext?.CardFragment?.CardDateAdapter?.UsersDateList;
        ...
        private void WebsiteButtonOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    IMethods.IApp.OpenbrowserUrl(this, Website);
                }
                else
                {
        ...
        private void SocialGoogleOnClick(object sender, EventArgs e)
        {
            try
            {
                if (IMethods.CheckConnectivity())
                {
                    IMethods.IApp.OpenbrowserUrl(this, "https://plus.google.com/u/0/" + Google);
                }
                else
                {
                    Toast.MakeText(this, GetString(Resource.String.Lbl_CheckYourInternetConnection), ToastLength.Short).Show();
        ...
        public void GetUserData(UserInfoObject dataUser)
        {
            try
            { 
                if (dataUser != null)
                {
                    ...
                    //Sent Api Visit
                    ApiRequest.GetInfoData(dataUser.Id.ToString()).ConfigureAwait(false);
